...
export function SessionStats({ stats }: SessionStatsProps) {
  ...
  return (
    <div className="glass flex items-center justify-center gap-6 rounded-xl px-4 py-2.5">
      <Stat icon="💬" label="Words" value={stats.wordsSpoken.toString()} />
      <div className="h-4 w-px bg-white/10" />
      <Stat icon="✏️" label="Corrections" value={stats.correctionsReceived.toString()} />
      <div className="h-4 w-px bg-white/10" />
      <Stat icon="⏱️" label="Time" value={elapsed} />
    </div>
  );
}
  ...
  return (
    <div className="flex items-center gap-1.5 text-center">
      ...
      <div>
        ...
        <p className="text-[10px] leading-none text-slate-500">{label}</p>
      </div>
    </div>
  );
}
